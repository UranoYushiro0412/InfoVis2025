d3.csv("https://UranoYushiro0412.github.io/InfoVis2025/W04/w04_task2.csv")
    .then( data => {
        ...
        d3.select('#ascend').on('click', () => {
            bar_chart.data.sort((a, b) => a.population - b.population);
            bar_chart.update();
        });

        d3.select('#descend').on('click', () => {
            bar_chart.data.sort((a, b) => b.population - a.population);
            bar_chart.update();
        });
    })
...
class BarChart {
    ...
    render() {
        ...
        self.xaxis_group.transition().duration(1500).call( d3.axisBottom(self.xscale).tickSizeOuter(0) );
        self.yaxis_group.transition().duration(1500).call( d3.axisLeft(self.yscale).ticks(5).tickSizeOuter(0) );
    }
}